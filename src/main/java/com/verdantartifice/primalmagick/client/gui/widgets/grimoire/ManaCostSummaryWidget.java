    ...
    public void renderWidget(MatrixStack matrixStack, int p_renderButton_1_, int p_renderButton_2_, float p_renderButton_3_) {
        ...
        if (this.isHovered()) {
            ...
            if (this.manaCosts.isEmpty()) {
                tooltip.add(new TranslationTextComponent("primalmagick.crafting.no_mana"));
            } else {
                tooltip.add(new TranslationTextComponent("primalmagick.crafting.mana_cost_header"));
                for (Source source : this.manaCosts.getSourcesSorted()) {
                    ...
                    tooltip.add(new TranslationTextComponent("primalmagick.crafting.mana_tooltip", this.manaCosts.getAmount(source), sourceText));
                }
            }

...
public class RitualRecipePage extends AbstractRecipePage {
    ...
    public void render(MatrixStack matrixStack, int side, int x, int y, int mouseX, int mouseY) {
        ...
        if (!this.recipe.getIngredients().isEmpty()) {
            ITextComponent leadComponent = new TranslationTextComponent("primalmagick.grimoire.ritual_offerings_header").mergeStyle(TextFormatting.UNDERLINE);
        ...
        if (!this.recipe.getProps().isEmpty()) {
            ITextComponent leadComponent = new TranslationTextComponent("primalmagick.grimoire.ritual_props_header").mergeStyle(TextFormatting.UNDERLINE);
            mc.fontRenderer.drawText(matrixStack, leadComponent, x - 3 + (side * 140), y - 6, Color.BLACK.getRGB());
            y += mc.fontRenderer.FONT_HEIGHT;
        ...
        ITextComponent headerComponent = new TranslationTextComponent("primalmagick.ritual.instability.header").mergeStyle(TextFormatting.UNDERLINE);
        int rating = MathHelper.clamp(this.recipe.getInstability() / 2, 0, 5);
        ITextComponent valueComponent = new TranslationTextComponent("primalmagick.ritual.instability.rating." + rating);
        ITextComponent lineComponent = new TranslationTextComponent("primalmagick.ritual.instability", headerComponent, valueComponent);

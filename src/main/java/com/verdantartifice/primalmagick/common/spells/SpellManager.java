    ...
    public static void cycleActiveSpell(@Nullable PlayerEntity player, @Nullable ItemStack wandStack, boolean reverse) {
        ...
        if (wandStack != null && wandStack.getItem() instanceof IWand) {
            ...
            if (player != null) {
                ...
                if (spell == null) {
                    player.sendMessage(new TranslationTextComponent("event.primalmagick.cycle_spell.none"), Util.DUMMY_UUID);
                } else {
                    player.sendMessage(new TranslationTextComponent("event.primalmagick.cycle_spell", spell.getName()), Util.DUMMY_UUID);
                }
            }
        }
    }
